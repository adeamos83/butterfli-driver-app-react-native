...
// Handles reconnecting to socket io once already logged in
export function reconnectSocketIo() {
  return (dispatch, store) => {
    console.log("reconnecting to the socket io server");
    socket = io(API_URL);
    socket.emit("LoggedIn", { payload: store().login.token });
    dispatch({
      type: "server/hello"
    });
  };
...
export function loginUser(email, password) {
  return (dispatch, store) => {
    return axios
      .post(DRIVER_SIGNIN_URL, { email, password })
      .then(response => {
      ...
      .catch(error => {
        ...
        errorLog(error);
        Actions.error_modal({
          data: "Could not log in. Please check username/password"
        });
      });
    // .then( async function(){
  ...
  return (dispatch, store) => {
    ...
    return axios
      ...
      .catch(error => {
        ...
        // }
        Actions.error_modal({
          data: "Could not sign up. Please check information and try again."
        });
        setTimeout(function() {
          dispatch(isSigningUp(false));
        }, 3000);
...
export function unAuthUser() {
  return (dispatch, store) => {
    if (
      store().home.driverStatus !== "available" &&
      store().home.driverStatus !== "notAvailable"
    ) {
...
export function resetPassword(email) {
  ...
  return dispatch => {
    ...
    return axios
      .post(FORGOT_PASSWORD_URL, forgotEmail)
      .then(response => {
...
export function createProfile() {
  return (dispatch, store) => {
    ...
    return axios
      .post(CREATE_PROFILE_URL, details, {
        headers: { authorization: "bearer " + store().login.token }
      })
      .then(response => {
      ...
      .catch(error => {
        ...
        Actions.error_modal({
          data:
            "Could not create your user profile. Please check information and try again."
        });
        dispatch(isSigningUp(false));
      });
  };
    ...
    return axios
      ...
      .catch(error => {
        ...
        if (error.response.status === 401) {
          ...
          Actions.error_modal({
            data:
              "Could not get retreive vehicles from server. Please wait a few minutes and try again."
          });
        }
      });
  };
